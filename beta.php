...
$PAGE->set_url('/local/kent/beta.php');
...
if ($form->is_cancelled()) {
    redirect($CFG->wwwroot . '/local/kent/beta.php');
}
...
if ($data = $form->get_data()) {
    ...
    $prefs = array();
    foreach ($arr as $k => $v) {
        if (strpos($k, 'isexpanded') === false) {
            $prefs[] = $k . "=" . ($v ? '1' : '0');
        }
    }

    set_user_preference("kent_betaprefs", implode(',', $prefs));
    redirect(new \moodle_url('/local/kent/beta.php'));
} else {
    // Set defaults.
...
//echo \html_writer::tag("p", "There are currently no beta programs running. Please check back later.");
